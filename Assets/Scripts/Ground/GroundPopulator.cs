...
public class GroundPopulator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (data.Type == GroundData.GroundType.Safe && data.EnigmePosition == GroundData.EnigmeZonePosition.None)
        {
            Debug.Log($"[GroundPopulator] Skipped population for safe ground sans énigme: {gameObject.name}");
            return;
        }
